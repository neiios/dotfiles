# Taken from: https://github.com/Smona/nixpkgs/blob/main/applications/nixGL.nix
...
# Call once on import to load global context
{
  ...
  config,
  nixGL,
  nixGLPrefix ? "${nixGL.packages.x86_64-linux.nixGLDefault}/bin/nixGL",
}:
...
(pkg.overrideAttrs (
  old: {
    ...
    # Seems to fix packages with debug outputs eg. easyeffects
