...
hl.bind("SUPER + SHIFT + S", hl.dsp.exec_cmd("uwsm app -- hyprshot -m region -z -s --clipboard-only"))
hl.bind("SUPER + Space", hl.dsp.exec_cmd("hyprctl switchxkblayout current next"))
hl.bind(
	"SUPER + SUPER_L",
	hl.dsp.exec_cmd(
		[[sh -c 'app=$("$HOME/.local/bin/tofi-drun" --drun-launch=false) && [ -n "$app" ] && exec uwsm app -- $app']]
	),
	{ release = true }
)
